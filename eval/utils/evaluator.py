...
class VQGANEvaluator:
    ...
    def reset_metrics(self):
        ...
        self._total_psnr = torch.tensor(0.0, dtype=torch.float64, device=self._device)
    ...
    def update(
        ...
        if self._enable_psnr:
            ...
            mse_batch = torch.mean((real_images - fake_images) ** 2, dim=[1, 2, 3])
            # Calculate PSNR for each image and sum them. Max pixel value is 1.0.
            psnr_batch = -10 * torch.log10(mse_batch + self._psnr_eps)
            self._total_psnr += torch.sum(psnr_batch).to(torch.float64)
    ...
    def result(self) -> Mapping[Text, torch.Tensor]:
        ...
        if self._enable_psnr:
            avg_psnr = self._total_psnr / self._num_examples
            eval_score["PSNR"] = avg_psnr.item()
