...
class VQGANEvaluatorVideo:
    def __init__(
        ...
        enable_token_length_measure: bool = False,
        enable_psnr: bool = False,
        ...
        self._enable_token_length_measure = enable_token_length_measure
        self._enable_psnr = enable_psnr
        self._enable_ssim = enable_ssim
        ...
        if self._enable_fvd:
            self._fvd_calculator = FVDCalculator()
            self._fake_stats = None
            self._running_real_stats = None
    ...
    def reset_metrics(self):
        ...
        self._token_number_mean, self._token_number_std = 0, 0
        
        # self._perplexity_count = torch.zeros(self.num_codebook_entries, dtype=torch.long, device=self._device)
    ...
    def update(
        ...
        token_number: Optional[torch.Tensor] = None,
    ):
        ...
        """
        real_images = rearrange(real_videos, "b c t h w -> (b t) c h w")
        fake_images = rearrange(fake_videos, "b c t h w -> (b t) c h w")
        ...
        if self._enable_fvd:
            if len(real_videos) > 128:
                for ii in range(0, len(real_videos), 128):
                    self._fake_stats = self._fvd_calculator.get_feature_stats_for_batch(fake_videos[ii:ii+128], self._fake_stats, num_gpus=1)
                    self._running_real_stats = self._fvd_calculator.get_feature_stats_for_batch(real_videos[ii:ii+128], self._running_real_stats, num_gpus=1)
            else:
                self._fake_stats = self._fvd_calculator.get_feature_stats_for_batch(fake_videos, self._fake_stats, num_gpus=1)
                self._running_real_stats = self._fvd_calculator.get_feature_stats_for_batch(real_videos, self._running_real_stats, num_gpus=1)

        if self._enable_psnr:
            psnr = calculate_psnr(real_images, fake_images)
            self._psnr_total += psnr.item()

        if self._enable_ssim:
            ssim = calculate_ssim(real_images, fake_images)
            self._ssim_total += ssim.item()
        ...
        if self._enable_token_length_measure and token_number is not None:
            self._token_number_mean += token_number.argmax(dim=1).float().mean()
            self._token_number_std += token_number.argmax(dim=1).float().std()
    ...
    def result(self) -> Mapping[Text, torch.Tensor]:
        ...
        if self._enable_fvd:
            eval_score["FVD"] = self._fvd_calculator.calculate_fvd(self._fake_stats, self._running_real_stats)

        if self._enable_psnr:
            eval_score["PSNR"] = self._psnr_total / self._num_examples

        if self._enable_ssim:
            eval_score["SSIM"] = self._ssim_total / self._num_examples
        ...
        if self._enable_token_length_measure:
            eval_score["TokenNumberMean"] = self._token_number_mean / self._num_updates
            eval_score["TokenNumberStd"] = self._token_number_std / self._num_updates
