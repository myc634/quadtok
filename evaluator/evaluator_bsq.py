...
class VQGANEvaluatorBSQ:
    ...
    def update(
        ...
        used_codes: Optional[torch.Tensor] = None,
        ...
        if self._enable_codebook_usage_measure:
            if self.all_codes is None:
                self.all_codes = used_codes
            else:
                self.all_codes = torch.cat([self.all_codes, used_codes], dim=0).unique()
    ...
    def result(self) -> Mapping[Text, torch.Tensor]:
        ...
        if self._enable_codebook_usage_measure:
            usage = float(self.all_codes.shape[0]) / (2 ** self._codebook_embed_dim)
