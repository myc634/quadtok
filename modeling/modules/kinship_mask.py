...
def build_batched_kinship_block_mask(lod_pad, parent_pad, seqlens, min_lod, device):
    """Batched (per-image) DECODER kinship BlockMask. lod_pad/parent_pad: [B,S] long, padding
    rows have lod=-1; seqlens: [B]. kv padding is masked out (finer would otherwise attend it)."""
    B, S = lod_pad.shape

    def mask_mod(b, h, q_idx, kv_idx):
        kv_valid = kv_idx < seqlens[b]
        lq = lod_pad[b, q_idx]; lk = lod_pad[b, kv_idx]
        finer = lk < lq
        same_level = (lk == lq) & ((lq == min_lod) | (parent_pad[b, q_idx] == parent_pad[b, kv_idx]))
        return kv_valid & (finer | same_level)

    return create_block_mask(mask_mod, B=B, H=None, Q_LEN=S, KV_LEN=S, device=device)


def build_batched_selector_block_mask(num_latent, lod_pad, parent_pad, seqlens, min_lod, device):
    """Batched (per-image) SELECTOR BlockMask over [num_latent latent] + [S tree] per image.
    latent<->latent full; tree->latent full; tree->tree kinship(valid); latent->tree none."""
    B, S = lod_pad.shape
    total = num_latent + S
    ...
    def mask_mod(b, h, q_idx, kv_idx):
        q_lat = q_idx < num_latent
        k_lat = kv_idx < num_latent
        qt = (q_idx - num_latent).clamp(min=0)
        kt = (kv_idx - num_latent).clamp(min=0)
        kv_tree_valid = (~k_lat) & (kt < seqlens[b])
        lq = lod_pad[b, qt]; lk = lod_pad[b, kt]
        finer = lk < lq
        same_level = (lk == lq) & ((lq == min_lod) | (parent_pad[b, qt] == parent_pad[b, kt]))
        tree_tree = kv_tree_valid & (finer | same_level)
        q_is_tree_row = k_lat | tree_tree          # tree query: sees all latent + valid kinship
        return torch.where(q_lat, k_lat, q_is_tree_row)  # latent query: sees only latent

    return create_block_mask(mask_mod, B=B, H=None, Q_LEN=total, KV_LEN=total, device=device)
