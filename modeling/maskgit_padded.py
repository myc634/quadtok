...
class ImageBertPadded(BaseModel, PyTorchModelHubMixin, tags=["arxiv:2304.12244"], pipeline_tag="text_to_image", license="mit"):
    def __init__(self, config):
        ...
        self.token_indices_embedding = nn.Embedding(total_patches, self.hidden_size)
        self.lod_incides_embedding = nn.Embedding(self.num_lod, self.hidden_size)

        # Initialize only the token and lod embeddings
        nn.init.normal_(self.token_indices_embedding.weight, mean=0.0, std=0.02)
        nn.init.normal_(self.lod_incides_embedding.weight, mean=0.0, std=0.02)
    ...
    def get_token_indices_embedding(self, tree_dict):
        ...
        embeddings = torch.zeros(batch_size, max_seq_len, self.hidden_size, 
    ...
    def forward(self, input_ids=None, condition=None, tree_dict=None, cond_drop_prob=0.1):
        # Token space:
    ...
    def masking_padded_input_tokens(self, input_tokens):
        batch_size, seq_len = input_tokens.shape
        device = input_tokens.device
        ...
        valid_token_number = valid_mask.sum(dim=1) 
    
        num_token_masked = (valid_token_number * mask_ratio).round().clamp(min=1)
        batch_randperm = torch.rand(batch_size, seq_len, device=device).argsort(dim=-1)
        batch_randperm = torch.where(valid_mask, batch_randperm, torch.tensor(float('inf'), device=device))
