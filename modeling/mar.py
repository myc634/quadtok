...
class QuadtreeGPT(BaseModel):
    ...
    def __init__(self, config):
        ...
        # --------------------------------------------------------------------------
        # MAR encoder specifics
        # codebook_size + 2: last two tokens are EXPAND_TOKEN and NO_EXPAND_TOKEN
        self.codebook_size = config.model.vq_model.codebook_size
        self.total_vocab_size = self.codebook_size + 2
        
        # lod=3 has 8x8=64 nodes for structure prediction
        self.num_structure_tokens = 64
        
        self.tok_embeddings = nn.Embedding(self.codebook_size, self.embed_dim)
        ...
        self.output = nn.Linear(self.embed_dim, self.total_vocab_size, bias=False)
        ...
        self.task_embedding = nn.Parameter(torch.randn(2, self.embed_dim))

        max_total_seq_len = 1 + self.seq_len * 2 + self.num_structure_tokens
        freqs_cis = precompute_freqs_cis(self.head_dim, max_total_seq_len)
        self.register_buffer("freqs_cis", freqs_cis)
    ...
    def forward(self, input_tokens, target_tokens, tree_dict, labels):
        ...
        device = target_tokens.device
        # get valid mask
        child_mask = tree_dict['child_mask']
        ...
        structure_embeddings = token_indices_embedding[:, :self.num_structure_tokens] + self.task_embedding[0].view(1, 1, -1).repeat(bs, 1, 1)
        token_embeddings = torch.cat((cond_embeddings, structure_embeddings, token_embeddings + token_indices_embedding[:, :-1] + self.task_embedding[1].view(1, 1, -1).repeat(bs, 1, 1)), dim=1)
        z = self.tok_dropout(token_embeddings)

        freqs_cis = self.freqs_cis[:max_seq_len + self.num_structure_tokens]
        freqs_cis = freqs_cis.unsqueeze(0).repeat(bs, 1, 1, 1)
        ...
        # 1: compute structure ce loss
        structure_logits = token_logits[:, :self.num_structure_tokens]
        structure_targets = child_mask.flatten() + self.codebook_size
        structure_loss = F.cross_entropy(structure_logits.flatten(0, 1).contiguous().float(), structure_targets.contiguous(), reduction="mean")

        token_loss = F.cross_entropy(token_logits[:, self.num_structure_tokens:][valid_mask].contiguous().float(), target_tokens[valid_mask].contiguous(), reduction="mean")

        total_loss = token_loss + 0.1 * structure_loss

        loss_dict = {}

        pred_tokens = torch.argmax(token_logits[:, self.num_structure_tokens:], dim=-1)
        acc = (pred_tokens == target_tokens)[valid_mask].float().mean()

        loss_dict['total_loss'] = total_loss.mean().detach()
        loss_dict['token_loss'] = token_loss.mean().detach()
        loss_dict['structure_loss'] = structure_loss.mean().detach()
    ...
    def generate(self,
                 ...
                 num_sample_steps=8):
        """
        Unified autoregressive generation in a single loop:
        - Step 0-63: Predict structure tokens (expand/no-expand)
        - Step 64+: Predict image tokens based on determined structure
        
        Sequence structure (same as training):
        [cls] + [structure_emb (64)] + [token_emb + pos_emb + task_emb[1]]
        
        Args:
            condition: Class labels of shape (batch_size,)
            guidance_scale: CFG guidance scale for image tokens
            structure_guidance_scale: CFG guidance scale for structure tokens
            guidance_decay: "constant" or "linear"
            randomize_temperature: Temperature for sampling
        
        Returns:
            result_tokens: Generated image tokens (list of tensors, one per batch)
            tree_list: List of generated tree structures
        """
        bsz = condition.shape[0]
        device = condition.device

        # Build base tree structure (lod=3, 8x8=64 nodes)
        tree_root = build_probabilistic_quadtree(
            self.num_patch_side_list, 
            guaranteed_depth=3, 
            expansion_probs=[0.5]
        ...
        for lod_idx, nodes in final_tree.items():
            if lod_idx == 3:
                for node in nodes:
                    ...
                    lod3_patch_indices.append(node.patch_index)

        lod3_patch_indices = torch.tensor(lod3_patch_indices, device=device, dtype=torch.long).unsqueeze(0).expand(bsz, -1)
        tree_dict = dict(
            lod_indices=torch.full_like(lod3_patch_indices, 3),
            patch_indices=lod3_patch_indices
        )

        # Get position embeddings for lod=3 nodes
        token_indices_embedding = self.get_token_indices_embedding(tree_dict)  # (bsz, 64, embed_dim)
        
        # Structure embeddings: pos_emb + task_embedding[0] (same as training)
        structure_embeddings = token_indices_embedding[:, :self.num_structure_tokens] + self.task_embedding[0].view(1, 1, -1)
        ...
        # Max sequence length: cls(1) + structure(64) + image(64 lod3 + up to 256 lod4)
        max_seq_len = 1 + self.num_structure_tokens + 64 + 256
        
        with torch.device(device):
            self.setup_caches(max_batch_size=cfg_bsz, max_seq_length=max_seq_len, dtype=class_embedding.dtype)
        
        # Storage for results
        structure_tokens = torch.zeros(bsz, self.num_structure_tokens, device=device, dtype=torch.long)
        
        # Initialize: start with class embedding
        x = class_embedding.repeat(1, self.cls_token_num, 1)
        cur_freqs_cis = self.freqs_cis[:self.cls_token_num].unsqueeze(0).repeat(cfg_bsz, 1, 1, 1)
        input_pos = torch.arange(0, self.cls_token_num, device=device)
        cache_position = self.cls_token_num
        
        # Variables for Phase 2 (will be set after structure prediction)
        image_pos_emb = None
        num_image_tokens = 0
        num_lod4_tokens_per_sample = None
        lod4_patch_indices_padded = None
        result_tokens = None
        
        # Total steps = structure tokens + image tokens (determined dynamically)
        total_steps = self.num_structure_tokens  # Will be extended after structure prediction
        step = 0
        
        while step < total_steps:
            # Forward pass
            logits = self.forward_inference(x, cur_freqs_cis, input_pos)  # (cfg_bsz, 1, total_vocab_size)
            
            if step < self.num_structure_tokens:
                # Structure tokens: use structure_guidance_scale
                if structure_guidance_scale != 1.0 and guidance_scale > 1.0:
                    cfg_iter = structure_guidance_scale  # constant for structure
                    cond_logits, uncond_logits = torch.chunk(logits, 2, dim=0)
                    logits = uncond_logits + cfg_iter * (cond_logits - uncond_logits)
                
                structure_logits = logits.clone()
                structure_logits[:, :, :self.codebook_size] = float('-inf')
                
                sampled_token = sample(structure_logits, randomize_temperature)[0].squeeze(1)  # (bsz,)
                structure_tokens[:, step] = sampled_token
                
                # Prepare next input
                if step < self.num_structure_tokens - 1:
                    # Next structure embedding
                    next_emb = structure_embeddings[:, step + 1:step + 2]  # (bsz, 1, embed_dim)
                else:
                    # Last structure step: determine tree structure and prepare for image generation
                    expand_mask = (structure_tokens == self.codebook_size + 1)  # (bsz, 64)
                    
                    # Calculate lod4 child indices
                    ...
                    expand_mask_4 = expand_mask.unsqueeze(-1).expand(-1, -1, 4)
                    
                    child_patch_indices_flat = child_patch_indices.reshape(bsz, -1)  # (bsz, 256)
                    expand_mask_flat = expand_mask_4.reshape(bsz, -1)  # (bsz, 256)
                    
                    num_lod4_tokens_per_sample = expand_mask_flat.sum(dim=1)  # (bsz,)
                    max_lod4_tokens = int(num_lod4_tokens_per_sample.max().item())
                    
                    # Sort to put valid tokens first
                    ...
                    if max_lod4_tokens > 0:
                        lod4_patch_indices_padded = gathered_indices[:, :max_lod4_tokens]
                        lod4_valid_mask = sorted_mask[:, :max_lod4_tokens].bool()
                    else:
                        lod4_patch_indices_padded = torch.zeros(bsz, 0, device=device, dtype=torch.long)
                        lod4_valid_mask = torch.zeros(bsz, 0, device=device, dtype=torch.bool)
                    
                    lod3_pos_emb = self.token_incides_embedding_dict['3'](lod3_patch_indices)  # (bsz, 64, embed_dim)
                    
                    if max_lod4_tokens > 0:
                        lod4_pos_emb = self.token_incides_embedding_dict['4'](lod4_patch_indices_padded)
                        lod4_pos_emb[~lod4_valid_mask] = 0
                        image_pos_emb = torch.cat([lod3_pos_emb, lod4_pos_emb], dim=1)
                    else:
                        image_pos_emb = lod3_pos_emb
                    image_pos_emb = image_pos_emb + self.task_embedding[1].view(1, 1, -1)
                    
                    num_image_tokens = 64 + max_lod4_tokens
                    total_steps = self.num_structure_tokens + num_image_tokens
                    
                    result_tokens = torch.zeros(bsz, num_image_tokens, device=device, dtype=torch.long)
                    next_emb = image_pos_emb[:, 0:1]  # (bsz, 1, embed_dim)
            else:
                # Image tokens: use guidance_scale with decay
                image_step = step - self.num_structure_tokens
                
                if guidance_scale != 1.0:
                    cfg_iter = self._get_cfg_scale(guidance_scale, guidance_decay, image_step, num_image_tokens, guidance_scale_pow, device)
                    cond_logits, uncond_logits = torch.chunk(logits, 2, dim=0)
                    logits = uncond_logits + cfg_iter * (cond_logits - uncond_logits)

                image_logits = logits.clone()
                image_logits[:, :, self.codebook_size:] = float('-inf')
                
                sampled_token = sample(image_logits, randomize_temperature)[0].squeeze(1)  # (bsz,)
                
                if image_step >= 64:
                    lod4_step_idx = image_step - 64
                    is_valid = lod4_step_idx < num_lod4_tokens_per_sample
                    sampled_token[~is_valid] = 0
                
                result_tokens[:, image_step] = sampled_token
                
                if step >= total_steps - 1:
                    break
                
                token_emb = self.tok_embeddings(sampled_token.unsqueeze(1))  # (bsz, 1, embed_dim)
                next_emb = token_emb + image_pos_emb[:, image_step + 1:image_step + 2]
            
            if guidance_scale != 1.0:
                next_emb = torch.cat([next_emb, next_emb], dim=0)
            
            x = next_emb
            cur_freqs_cis = self.freqs_cis[cache_position:cache_position + 1].unsqueeze(0).repeat(cfg_bsz, 1, 1, 1)
        ...
        for b in range(bsz):
            valid_len = 64 + int(num_lod4_tokens_per_sample[b].item())
            final_token_list.append(result_tokens[b, :valid_len])
            
            # Build tree structure
            batch_node_list = copy.deepcopy(base_node_list)
            valid_lod4_indices = lod4_patch_indices_padded[b, :int(num_lod4_tokens_per_sample[b].item())]
            for patch_idx in valid_lod4_indices:
                new_node = QuadTreeNode(lod_level=4, patch_index=patch_idx.item())
                batch_node_list.append(new_node)
            
            final_tree_list.append(batch_node_list)
        
        return final_token_list, final_tree_list
    
    def _get_cfg_scale(self, guidance_scale, guidance_decay, step, max_seq_len, guidance_scale_pow, device):
        """Helper function to compute CFG scale based on decay schedule."""
        if guidance_decay == "linear":
            return 1 + (guidance_scale - 1) * step / max_seq_len
        elif guidance_decay == "constant":
            ...
            return (guidance_scale - 1) * scale_step + 1
        else:
            return guidance_scale