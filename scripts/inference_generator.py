...
def main(args):
    ...
    if args.output_dir is None:
        ...
        output_dir_name = f"scale_{gs_str}_pow_{gs_pow_str}_decay_{guidance_decay_str}_temp_{config.model.generator.randomize_temperature:.2f}_num_{local_num_samples}"
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16, enabled=True) and torch.no_grad():
        for batch_idx in range(num_batches):
            ...
            batch_labels = torch.randint(0, config.model.generator.class_num, (current_batch_size,), device=device).long()
