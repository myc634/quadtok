...
def evaluate_tokenizer(config_path, model_type, data_path, output_path):
    ...
    if model_type == 'vavae':
        ...
        model = AutoencoderKL(embed_dim=16, ch_mult=[1, 1, 2, 2, 4], ckpt_path="pretrained_weight/vae/kl16.ckpt").to(device)
    ...
    pipeline = [
                wds.SimpleShardList("/mnt/shared-storage-user/idc2-shared/dataset/preprocess/imagenet/ILSVRC2012_validation/val-{000000..000049}.tar"),
                wds.split_by_node, 
                wds.split_by_worker,
                wds.tarfile_to_samples(handler=wds.ignore_and_continue),
                *test_processing_pipeline,
                wds.batched(32, partial=True, collation_fn=default_collate),
            ]
    eval_dataset = wds.DataPipeline(*pipeline)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_type', type=str, default='marvae')
    parser.add_argument('--data_path', type=str, default='/mnt/shared-storage-user/idc2-shared/dataset/preprocess/imagenet/ILSVRC2012_validation')
    parser.add_argument('--output_path', type=str, default='output_ours_w_interpolation')
