...
def eval_reconstruction(
    ...
    for batch in tqdm(eval_loader):
        ...
        reconstructed_images2 = model.decoder._forward_optimize(embeds, node_list)
        reconstructed_images2 = torch.clamp(reconstructed_images2, 0.0, 1.0)
        reconstructed_images1 = torch.cat([reconstructed_images, reconstructed_images_reversed], dim=0)
        for i in range(reconstructed_images1.shape[0]):
            diff_map = torch.abs(reconstructed_images1[i] - reconstructed_images2[i]).sum(0).repeat(3, 1, 1) / 4
            concat_image = torch.cat([reconstructed_images1[i], reconstructed_images2[i], diff_map], dim=2)
            concat_image = (concat_image * 255).cpu().numpy()
            concat_image = Image.fromarray(concat_image.transpose(1, 2, 0).astype(np.uint8))
            concat_image.save(f"./concat_image_{i}.png")
        breakpoint()
        ...
        '''reconstructed_images_valid = []
        for batch_idx in range(images.shape[0]):
            cur_z = model.selector._forward_reconstruction(image_latent[batch_idx:batch_idx+1], all_search_nodes[batch_idx])
            _, result_dict_search_cur = model.quantize(cur_z)
            token_indices_search_cur = result_dict_search_cur['min_encoding_indices']
            cur_embeds = model.quantize.get_codebook_entry(token_indices_search_cur.squeeze().long().flatten()).reshape(1, -1, 8)
            cur_reconstructed_images = model.decoder._forward_reconstruction(cur_embeds, all_search_nodes[batch_idx])
            cur_reconstructed_images = torch.clamp(cur_reconstructed_images, 0.0, 1.0)
            reconstructed_images_valid.append(cur_reconstructed_images)
        
        reconstructed_images_valid = torch.cat(reconstructed_images_valid, dim=0)

        for i in range(images.shape[0]):
            diff_map = torch.abs(reconstructed_images_valid[i] - reconstructed_images_search[i]).sum(0).repeat(3, 1, 1) / 4
            concat_image = torch.cat([images[i], reconstructed_images_search[i], reconstructed_images_valid[i], diff_map], dim=2)
            concat_image = (concat_image * 255).cpu().numpy()
            concat_image = Image.fromarray(concat_image.transpose(1, 2, 0).astype(np.uint8))
            concat_image.save(f"./concat_image_{i}.png")'''
        ...
        evaluator.update(images, reconstructed_images_search.squeeze(2), token_indices_search.squeeze().long())
...
def main(args):
    ...
    dataset = SimpleImageDataset(
        ...
        eval_shards_path="/mnt/ultracube/datasets/imagenet-1k-wds-val/imagenet1k-validation-{00..02}.tar",
        **base_params,
    )
