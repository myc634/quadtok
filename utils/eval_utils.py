...
INCEPTION_V3_PATH = "pretrained_weight/classify_image_graph_def.pb"
...
def create_npz_from_sample_folder(sample_dir, num=50_000):
    ...
    png_files = sorted(sample_dir_path.glob("*.png"))
    
    if len(png_files) == 0:
        raise ValueError(f"No .png files found in {sample_dir}")
    
    # If num is specified and less than total files, limit the number
    if len(png_files) > num:
    ...
    def frechet_distance(self, other, eps=1e-6):
        ...
        if np.iscomplexobj(covmean):
            if not np.allclose(np.diagonal(covmean).imag, 0, atol=1e-3):
                ...
                print("WARN when computing FID", "Imaginary component {}".format(m))
                print("return 0 as fake-FID")
                return 0
